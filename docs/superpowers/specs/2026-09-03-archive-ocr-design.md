...
- **실패한 레코드는 목록 시트에서 행에 배경색을 깐다.** 목록만 보면 제목이 정상이라
  사진이 빠진 것을 알 수 없기 때문이다. 평가양식이 4열로 고정이므로 열을 늘리지 않고
  배경색으로만 표시한다 — 열 구성과 값은 그대로 유지된다.
  두 갈래를 색으로 구분한다: OCR 실패는 붉은색, 업로드 실패는 호박색. 둘 다 실패하면
  붉은색으로 표시한다(전사가 없는 쪽이 더 근본적인 결손).
